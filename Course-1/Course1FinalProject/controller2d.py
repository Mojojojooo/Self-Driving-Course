...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            #integral value
            e_int = self.vars.integral_error_previous + e_value * st

            #differential value
            e_diff = (e_value - self.vars.error_previous)/ st

            #accelation
            acc = kp * e_value + ki * e_int + kd * e_diff #using final PID equation
            ...
            steer_expected = yaw_diff_crosstrack + yaw_diff_heading
            if steer_expected > np.pi:
                steer_expected -= 2 * np.pi
            if steer_expected < - np.pi:
                steer_expected += 2 * np.pi
            steer_expected = min(1.22, steer_expected)
            steer_expected = max(-1.22, steer_expected)

            #update
            steer_output = steer_expected
        ...
        self.vars.error_previous = e_value
        self.vars.integral_error_previous = e_int
